...
//@Autonomous(name="driveEnc", group="MyGroup")
public class driveEnc extends LinearOpMode{
    ...
    public void runForEnc(int ticks, double power) {
        ...
        while(getAllDriveEncAvg() <= ticks && opModeIsActive()) {
            ...
            frontLeft.setPower(Range.clip(power - leftRightDiff, 0, 1));
            frontRight.setPower(Range.clip(power - leftRightDiff, 0, 1));
            backLeft.setPower(Range.clip(power - leftRightDiff, 0, 1));
            backRight.setPower(Range.clip(power - leftRightDiff, 0, 1));
