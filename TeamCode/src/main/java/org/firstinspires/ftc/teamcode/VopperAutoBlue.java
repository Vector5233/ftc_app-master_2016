...
@Autonomous(name="VopperAutoBlue", group="MyGroup")
public class VopperAutoBlue extends LinearOpMode{
    ...
    */
    ...
    // Rotate bot to be parallel with wall, 0 degrees from starting position (because we're blue)
    ...
    public void alignBopperWithButtons() {
        ...
        if (colorConfig == State.BLUE_RED) {
            // We're blue, so we're aiming for the right button (from the front)

            runMM(RIGHT_BEACON_MM); //-98
        }
        else { //RED_BLUE or weird decision
            // Aiming for the right button (from the front)

            runMM(LEFT_BEACON_MM); //-230
        }
    ...
    public void driveToSecondBeacon() {
        // We're blue, so if BLUE_RED, we need to drive a shorter distance to the next image

        //4278 is the number of ticks from the center of one image to the center of the next
